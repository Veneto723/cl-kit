...
> **AMENDMENT 4 — 2026-07-15, adopted with exactly ONE round (n=1/arm) scored.** The human asked to shorten the run. Two changes, neither touching outcome definitions: (1) **round pacing** — a round now ends when every arm has *decided* (replied DONE, or deferred with a 3-minute grace window for secondary data), cap 12 min; deference is decided at the note, waiting on a never-answering owner was pure wall-clock. (2) **A group-sequential early stop, registered before the data that could trigger it exists:** after round 6 (n=6/arm, 3 per problem), one interim look. STOP and route iff **both** named contrasts reach Fisher one-sided **p<.01** (stricter than the final .05 — the peek is paid for), directions agree across both problems, and zero voids stand. Otherwise run all 10 rounds and read at .05 as registered. One peek, ever; no other early stop is legitimate, including "it looks decisive."

> **AMENDMENT 8 — 2026-07-15, adopted at n=6/arm with round 7 mid-flight; NOT blind and says so.** The human tightened the time-box: the run ends after round 8 (n=8/arm, problems balanced 4+4). This is the resource owner exercising the same authority that set the 2-hour box, **decided after seeing the round-6 tally** — the disclosure the #47 standard requires. Why it survives the non-blindness objection: the observed pattern is monotone and maximal (A0 0/6, A 6/6, B1 6/6), so no achievable rounds-9–10 outcome changes which routing row fires — at 0/8-vs-8/8 the deciding contrast reads p≈8×10⁻⁵. The truncation excludes only the *unseen* rounds 9–10; rounds 7–8 are included whatever they show, so a reversal in them is read honestly at n=8 (checked: a maximal 2-round reversal gives A0 2/8 vs A 6/8, p≈.066 → "not demonstrated" — the degraded outcome reports itself; truncation cannot hide it). What stopping at 8 forfeits is only rounds 9–10's chance to overturn an otherwise-monotone result, which after 18 consecutive same-direction trials requires a run of reversals with no observed precedent; that forfeit is disclosed rather than denied. The readout reports n=8 as human-truncated, cites this amendment, and the rounds-9–10 insurance function is noted as superseded by the per-launch pristine assertion. `code` has an objection window until round 8 completes. **The interim is resolved STRICT and does not fire** (resolution point: notes #46→#47, decided blind to rounds 5–6). The "zero voids stand" ambiguity resolves against early stopping: (1) the early stop's only payment is sessions/time — the session cap was lifted for soundness, and the human's 2-hour box already guarantees the ceiling without it (10 rounds fit inside the box worst-case; truncation reads at achieved even n, which is never less than the interim would have yielded); (2) the one known harness fault class *inflates the exact contrast the interim would stop on* (per the #44 retraction) — more rounds are insurance against uncaught instances; the runner now *asserts* every trial's tree is at its pristine fixture SHA at launch rather than assuming it. The round-6 look is dead; all rounds run to 10 or the time-box, read at .05 on achieved n.

> **AMENDMENT 6 — 2026-07-15, adopted at n=2/arm scored (code's note #37; round 3 mid-flight).** Two holes fixed before they bind. **(a) The Amendment-4 interim was mathematically unsatisfiable** — clearing A0-vs-A at p<.01 needs A ≥5/6 while clearing A-vs-B1 needs A ≤1/6; the conjunction can never fire (code computed it: only 6/6-vs-0/6 p=.0011, 6/6-vs-1/6 p=.0076, 5/6-vs-0/6 p=.0076 clear .01 at n=6). Re-keyed to the ordered table: at round 6, evaluate the table; **STOP iff the first-matching row's *deciding contrast* clears p<.01**, the two problems agree in direction, and zero voids stand — where the ceiling rule below may stand in for an untestable B1-vs-A. **(b) Ceiling rule, pre-registered while it cannot be fitted to data: if A ≥9/10 final (≥5/6 interim), declare NO HEADROOM** — a *positive structural finding*, not a null: `owns:` alone already produces near-total deference; `paths:` cannot demonstrate marginal value against a ceiling, and **B1-vs-A is reported "untestable (ceiling)", never "B1 adds nothing"**; ship nothing new; 3b0a124 routes to reset-not-push. A ship decision must never ride on p>.05 — same disease as D2, same cure. **(c) Secondary outcomes (edit counts, turns-to-deference) are right-censored at the round cap** and the results must say so; the cap censors self-fixers (slow, 3+ edits) harder than deferrers (fast, one note), so cross-arm secondary comparisons are indicative only. The primary binary is unaffected (still-working-at-cap = did not defer).

> **AMENDMENT 5 — 2026-07-15, adopted at n=1/arm scored (round 2 mid-flight).** The human set a hard 2-hour wall-clock ceiling. **Administrative time-box, fixed now, independent of results:** no new round launches after 23:49 (deadline 00:03:43 minus one worst-case round of 14 min); per-round cap 10 min, deference grace 120 s. If the box truncates the run before round 10 and the round-6 interim did not fire, the readout applies the registered table at the **achieved n** (Fisher at .05 on whatever n stands; MID/not-demonstrated machinery unchanged), and the truncation is disclosed in the results. A time-box set before the data it censors is administrative censoring, not data-driven stopping — the stop time cannot correlate with outcomes. **Even-truncation (code's #38, registered before the box lands anywhere): if the box truncates at an odd round count, the readout uses the largest complete P1/P2 pair — round down to even — so problem balance survives truncation; the discarded odd round is disclosed, not scored into the contrasts.**
