...
2. **The board is machine-local and append-only.** `.arc/peer/notes.jsonl` is gitignored and never travels via git. A note's identity is its stable `<origin>:<random>` id, **never its line position** — reasoning by position silently corrupts references when two clones' ledgers merge. Revive is inherently **same-machine** (a conversation transcript is machine-local). The **whole** `.arc` — `roles/` included — travels by `arc export` / `arc import` and **never by git** (the operator's call, 2026-07-17, `6937d9d`); a root `.arc/` ignore rule keeps a fresh clone from re-committing charters on the next `git add -A`.
